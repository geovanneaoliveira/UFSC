...
Gs_Subamortecido = GeraFT_2(K, wn, ksi, L);
[K, ksi, wn, L] = lab2_funcs(y, A, fs, 'Mollenkamp');
Gs_Mollenkamp = GeraFT_2(K, wn, ksi, L);


y_teorica = lsim(GsTeorica, u, tempo);
y_real_modelo = lsim(GsReal, u, tempo);


figure
plot(tempo, y_teorica, '--b')
hold on
plot(tempo, y_real_modelo, '--r')
grid on

legend('Teórico', 'Modelo Real')
xlabel('Tempo (s)')
ylabel('Amplitude')
title('Comparação dos sinais de saída')
